...
func (repo *Extractor) GetPostWithCode(code string) (domain.Media, error) {
	...
	if coverPhoto != "" {
		unescaped := unescapeUnicodeRegex(unescapeUnicode(coverPhoto))
		fmt.Println("Cover photo URL:", unescaped)
		return domain.Media{
			URL:     unescaped,
...
func main() {
	...
	code, _ := ExtractShortCodeFromLink("https://www.instagram.com/reels/DJMmTZ4onhr/") // Replace with actual URL

	// code = "thereallnico"
	fmt.Printf("Fetching post with code: %s\n", code)

	media, err := extractor.GetPostWithCode(code)
	if err != nil {
		logging.Errorf("Error fetching post with code %s: %v", code, err)
		return
	}

	js, _ := json.MarshalIndent(media, "", "  ")
	fmt.Printf("Fetched media: %s", (string(js)))
...
func unescapeUnicode(s string) string {
	...
	for i := 0; i < len(s); {
		if i+5 < len(s) && s[i:i+2] == "\\u" {
			// Try to parse the Unicode escape sequence
			if r, _, _, err := strconv.UnquoteChar(s[i:i+6], 0); err == nil {
				result.WriteRune(r)
				i += 6
			} else {
...
func unescapeUnicodeRegex(s string) string {
