...
func main() {
	...
	// The actual functionality is implemented in the extractor methods.
	// You can run tests or examples here if needed.
	// For example, you can call the Extract method with a sample URL.
	url := "https://vk.com/video-209450022_456239483"
	extractor := New()
	data, err := extractor.Extract(url, extractors.Options{})
	if err != nil {
		panic(err)
	}
	for _, d := range data {
		println("Extracted video title:", d.Title)
	}
}
