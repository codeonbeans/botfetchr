...
SET account_id = COALESCE(sqlc.narg('account_id'), account_id),
    ...
    end_date   = CASE
                   WHEN sqlc.arg('null_end_date')::boolean THEN NULL
                   ELSE COALESCE(sqlc.narg('end_date'), end_date) END,
    cancel_at  = CASE
                   WHEN sqlc.arg('null_cancel_at')::boolean THEN NULL
                   ELSE COALESCE(sqlc.narg('cancel_at'), cancel_at) END
